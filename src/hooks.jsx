...
function HookEditor({ hook, onSave, onDelete }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-3 gap-3">
      <label className="block text-sm">Event<select className="block mt-1 w-full border rounded px-3 py-2" value={draft.event} onChange={event => updateDraft({ event: event.target.value })}><option value="tool.call">tool.call</option><option value="agent.run">agent.run</option></select></label>
    ...
    <div className="text-xs text-gray-500 space-y-1"><p>函数格式：<code>async (&#123; phase, context, state &#125;) =&gt; &#123; ... &#125;</code></p><p><code>phase</code>：before / after / error / cancel；<code>context.data</code>：当前流程数据；<code>state</code>：before 返回、只回传给本 Hook 的临时数据。</p><p>工具调用的 <code>context.data</code> 包含 <code>tool</code>、<code>args</code>、<code>result</code>；Agent 运行包含 <code>input</code>；可使用 <code>fetch</code>、<code>chrome</code>、<code>llm.profiles()</code> 和 <code>llm.complete()</code>。</p></div>
  </div>;
}

...
const CODE_MODE_TOOLS = [
  {
    ...
    description:
      ...
      "Built-in tools are called as `await tools.tool_name(args)`; MCP tools are normally called as `await tools.mcp.server_name.tool_name(args)`. If an MCP server or tool name contains a hyphen or any other character that is not valid in a JavaScript identifier, use bracket notation for that property, for example `await tools.mcp['server-name']['tool-name'](args)`; otherwise JavaScript parses the hyphen as subtraction. Treat a non-null MCP outputSchema as server-declared; when it is null, inspect the actual result instead of guessing fields. " +
...
export const TOOLS = [
  ...
  },
  {
    ...
    description: "Create and open a storage-backed HTML/CSS/JS playground, or reopen one by playgroundId. Keep expanded false by default; set expanded true only when the user explicitly requests the source editors or wants to inspect detailed code. Write HTML, CSS, and JavaScript as readable multi-line source with normal indentation; never minify or collapse generated code into one line, because users inspect it in the editors. New projects expire after 24 hours by default and may specify expireAt. The result returns a stable playgroundId, expireAt, and a files array containing absolute VFS paths for index.html, style.css, and script.js. Pass any returned files path unchanged to vfs_read_file, vfs_edit_file, or vfs_write_file. Prefer vfs_edit_file for targeted changes and vfs_write_file only when replacing the complete file. Pass playgroundId without html/css/js to reopen an existing project.",
    schema: {
      type: "object",
      properties: {
        ...
        expanded: { type: "boolean", description: "Whether the HTML/CSS/JS editor inputs should be expanded initially. Defaults to false; use true only when the user explicitly asks to inspect or show the detailed source code." }
      },
      required: []
    }
  },
  ...
  {
    name: "webide_project",
    description: "Create and open a multi-file browser WebIDE project, or reopen one by projectId. Use template react for React JSX applications or vanilla for plain JavaScript applications. The project is stored in VFS, transformed through esm.sh in a sandbox, and rendered as a live native-ESM preview. package.json is used only to create an import map: no npm install or package scripts run, and every imported bare package must be listed under dependencies with an exact version (do not use latest, ^, or ~). Third-party package roots are bundled by esm.sh with react/react-dom externalized to the project versions; import peer-dependent component libraries from their package root, for example `import { Button } from \"antd\"`, and declare exact versions for react, react-dom, and the library. Deep subpath imports may not receive the same bundling treatment. Libraries that depend on Node.js APIs, bundler plugins/loaders, compile-time code generation, or other non-browser features are unsupported. Remote package CSS imports such as `import \"antd/dist/reset.css\"` are unsupported; add an explicit CDN stylesheet link to index.html instead. Local JS, JSX, TS, TSX, CSS, and JSON files are supported. The result returns absolute VFS file paths; use vfs_read_file, vfs_edit_file, and vfs_write_file to modify or add readable multi-line project files. New projects expire after 24 hours by default.",
    schema: {
      type: "object",
      properties: {
